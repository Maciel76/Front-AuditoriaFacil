...
function apiPortal() {
  return {
    get:  (url, cfg)  => api.request({ method: 'get',  url, ...cfg, headers: { Authorization: `Bearer ${token.value}` } }),
    post: (url, d, cfg) => api.request({ method: 'post', url, data: d, ...cfg, headers: { ...cfg?.headers, Authorization: `Bearer ${token.value}` } }),
  };
}
...
async function verificar() {
  if (!matricula.value.trim()) {
    erro.value = 'Informe sua matrícula.'; return;
  }
  ...
  try {
    ...
    primeiroNome.value = data.nome || lojasDisponiveis.value[0]?.primeiroNome || '';
    etapa.value = 'selecionar';
  } catch (e) {
...
async function configurarSenha() {
  if (!lojaSlug.value.trim()) { erro.value = 'Selecione uma loja para continuar.'; return; }
  if (senha.value.length < 6) { erro.value = 'Mínimo de 6 caracteres.'; return; }
  if (senha.value !== senhaConfirm.value) { erro.value = 'As senhas não coincidem.'; return; }
...
async function login() {
  if (!lojaSlug.value.trim()) { erro.value = 'Selecione uma loja para continuar.'; return; }
  if (!senha.value) { erro.value = 'Informe a senha.'; return; }
  carregando.value = true; erro.value = '';
  try {
...
async function carregarPerfil() {
  const { data: perfilData } = await apiPortal().get('/colaboradores/portal/me');
  ...
  if (perfilData.loja) {
    lojaSelecionada.value = {
      ...
      cidade: lojaSelecionada.value?.cidade || '',
      estado: lojaSelecionada.value?.estado || '',
...
// Avatar
function abrirAvatar() { avatarInput.value?.click(); }
...
async function confirmarCropAvatar() {
  ...
  try {
    ...
    fd.append('avatar', new File([blob], `avatar-${perfil.value._id}.jpg`, { type: 'image/jpeg' }));

    const res = await apiPortal().post(`/colaboradores/${perfil.value._id}/avatar`, fd, {
    ...
    erroConfig.value = e?.response?.data?.error || e?.message || 'Erro ao atualizar a foto.';
  } finally {
...
async function alterarSenhaConta() {
  ...
  try {
    ...
    erroConfig.value = e?.response?.data?.error || 'Não foi possível atualizar sua senha.';
  } finally {
...
// --- Conquistas emojis ---
const conquistaIco = {
  PRIMEIRA_AUDITORIA: '🏁', CEM_LIDOS: '💯', MIL_LIDOS: '🔢',
  DEZ_MIL_LIDOS: '⭐', CONFORMIDADE_95: '🥇', NIVEL_5: '🎖️',
};

// --- Charts ---
const corPorTipo = { ETIQUETA: '#7c5cff', PRESENCA: '#22d3ee', RUPTURA: '#f59e0b' };
const serieComoColunas = computed(() => (metricas.value?.serie?.length || 0) <= 12);
...
onMounted(async () => {
  temaAnterior = document.documentElement.getAttribute('data-theme') || localStorage.getItem('na_tema') || 'dark';
...
onBeforeUnmount(() => {
  destruirCropper();
  document.documentElement.setAttribute('data-theme', localStorage.getItem('na_tema') || temaAnterior || 'dark');
});
  ...
  <div class="portal-shell">
    ...
    <header class="portal-header">
      <div class="portal-brand">
        ...
        <span class="brand-name">NovaAuditoria <small>Portal do Colaborador</small></span>
      </div>
    ...
    <div v-if="etapa === 'buscar'" class="portal-card">
      ...
      <p class="muted" style="text-align:center; font-size: 14px; margin: 0 0 24px;">
        Informe sua matrícula para localizar as lojas em que você possui auditorias.
      </p>
      <div class="grid gap-3">
        <div class="field">
          <label>Matrícula</label>
          <input v-model="matricula" placeholder="ex.: 2692473" @keyup.enter="verificar" />
        </div>
        <div v-if="erro" class="badge bad" style="width:100%; justify-content:center;">{{ erro }}</div>
        <button class="btn primary" :disabled="carregando" @click="verificar" style="width:100%;">
          <fa :icon="carregando ? 'spinner' : 'arrow-right'" :spin="carregando" />
          Ver minhas lojas
        </button>
      </div>
    </div>
    ...
    <!-- ======= ETAPA: ESCOLHER LOJA ======= -->
    <div v-else-if="etapa === 'selecionar'" class="portal-card portal-card-wide">
      <div class="center mb-4">
        <div class="brand-mark big"><fa icon="store" /></div>
      </div>
      <h2 style="text-align:center; margin: 0 0 6px;">Escolha a loja</h2>
      <p class="muted" style="text-align:center; font-size: 14px; margin: 0 0 24px;">
        {{ primeiroNome }}, encontramos {{ lojasDisponiveis.length }} {{ lojasDisponiveis.length === 1 ? 'loja' : 'lojas' }} para a matrícula {{ matricula }}.
      </p>
      <div class="store-options">
        ...
        >
          <div class="store-option-main">
            <div class="store-option-icon"><fa icon="store" /></div>
            <div class="store-option-copy">
              <strong>{{ loja.nomeLoja }}</strong>
              <span class="muted">{{ localidadeLoja(loja) || 'Loja disponível no portal' }}</span>
              <div class="row gap-1" style="margin-top: 6px;">
                <span class="badge" :class="loja.primeiroAcesso ? 'warn' : 'info'">
                  {{ loja.primeiroAcesso ? 'Primeiro acesso' : 'Senha já cadastrada' }}
                </span>
                <span v-if="loja.slug === lojaPreferencial" class="badge dim">link atual</span>
              </div>
            </div>
          </div>
      ...
      <button class="btn ghost" @click="voltarParaBusca" style="width:100%; margin-top: 16px;">Informar outra matrícula</button>
    </div>

    <!-- ======= ETAPA: PRIMEIRO ACESSO ======= -->
    <div v-else-if="etapa === 'setup'" class="portal-card">
      <div v-if="lojaSelecionada" class="selected-store">
        <span class="badge info"><fa icon="store" /> {{ lojaSelecionada.nomeLoja }}</span>
        <span class="muted" style="font-size:12px;">{{ resumoLojaSelecionada }}</span>
      </div>
      ...
      <div class="grid gap-3">
        <div class="field">
          ...
          <input type="password" v-model="senha" placeholder="••••••" @keyup.enter="configurarSenha" />
        </div>
        <div class="field">
          <label>Confirmar senha</label>
          <input type="password" v-model="senhaConfirm" placeholder="••••••" @keyup.enter="configurarSenha" />
        </div>
        <div v-if="erro" class="badge bad" style="width:100%; justify-content:center;">{{ erro }}</div>
        <button class="btn primary" :disabled="carregando" @click="configurarSenha" style="width:100%;">
          <fa :icon="carregando ? 'spinner' : 'check'" :spin="carregando" />
          Definir senha e entrar
        </button>
        <button class="btn ghost" @click="voltarParaSelecao" style="width:100%;">Trocar loja</button>
      </div>
    </div>
    ...
    <div v-else-if="etapa === 'login'" class="portal-card">
      ...
      <p class="muted" style="font-size: 14px; margin: 0 0 24px;">Informe sua senha para entrar.</p>
      <div class="grid gap-3">
        <div class="field">
          <label>Senha</label>
          <input type="password" v-model="senha" placeholder="••••••" autofocus @keyup.enter="login" />
        </div>
        <div v-if="erro" class="badge bad" style="width:100%; justify-content:center;">{{ erro }}</div>
        <button class="btn primary" :disabled="carregando" @click="login" style="width:100%;">
          <fa :icon="carregando ? 'spinner' : 'right-to-bracket'" :spin="carregando" />
          Entrar
        </button>
        <button class="btn ghost" @click="voltarParaSelecao" style="width:100%;">Trocar loja</button>
      </div>
    ...
    <div v-else-if="etapa === 'portal' && perfil" class="portal-content">
      ...
      <div class="card glow portal-hero">
        <div class="hero-avatar">
          <div class="avatar-big" @click="abrirAvatar" title="Trocar foto" style="cursor:pointer;">
            <img v-if="perfil.avatarUrl" :src="perfil.avatarUrl" alt="foto" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" />
            <span v-else>{{ iniciais }}</span>
            <div class="avatar-overlay"><fa icon="camera" /></div>
          </div>
          <input ref="avatarInput" type="file" accept="image/*" hidden @change="enviarAvatar" />
          <div v-if="enviandoAvatar" class="muted" style="font-size:12px; margin-top:6px;">Enviando…</div>
        </div>
        <div class="hero-info">
          ...
          <div class="row gap-2 mb-2" style="flex-wrap:wrap;">
            ...
            <span v-if="perfil.cargo" class="badge info">{{ perfil.cargo }}</span>
            <span v-if="perfil.setor" class="badge dim">{{ perfil.setor }}</span>
          </div>
          <!-- Nível e progresso -->
          <div class="row gap-3">
            ...
            <div style="flex:1;">
              <div class="row mb-1" style="font-size: 12px;">
                <span class="muted">{{ perfil.pontuacao.toLocaleString('pt-BR') }} pts</span>
                <span class="spacer" />
                <span class="muted">Próx. nível: {{ pontuacaoProxNivel.toLocaleString('pt-BR') }}</span>
              </div>
              <div class="progress"><span :style="{ width: pctNivel + '%' }" /></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="portal-settings-grid">
        <div class="card portal-settings-card">
          ...
          <div class="settings-block">
            ...
            <p class="muted settings-copy">O portal abre em tema claro por padrão. Você pode trocar quando quiser.</p>
            <div class="theme-toggle-group">
              <button class="theme-toggle" :class="{ active: temaPortal === 'light' }" @click="aplicarTemaPortal('light')">
                <fa icon="sun" /> Claro
              </button>
              <button class="theme-toggle" :class="{ active: temaPortal === 'dark' }" @click="aplicarTemaPortal('dark')">
                <fa icon="moon" /> Escuro
              </button>
            </div>
          </div>

          <form class="settings-block grid gap-3" @submit.prevent="alterarSenhaConta">
            <div>
              <div class="settings-title">Segurança</div>
              <p class="muted settings-copy">Atualize sua senha sempre que quiser, usando sua senha atual para confirmar.</p>
            </div>
            <div class="field">
              <label>Senha atual</label>
              <input v-model="senhaAtualConta" type="password" placeholder="••••••" autocomplete="current-password" />
            </div>
            <div class="field">
              <label>Nova senha</label>
              <input v-model="novaSenhaConta" type="password" placeholder="••••••" autocomplete="new-password" />
            </div>
            <div class="field">
              <label>Confirmar nova senha</label>
              <input v-model="confirmarNovaSenhaConta" type="password" placeholder="••••••" autocomplete="new-password" />
            </div>
            <div v-if="erroConfig" class="badge bad settings-feedback">{{ erroConfig }}</div>
            <div v-else-if="sucessoConfig" class="badge ok settings-feedback">{{ sucessoConfig }}</div>
            <button class="btn primary" type="submit" :disabled="alterandoSenha">
      ...
      <div class="card" v-if="perfil.conquistas?.length">
        ...
        <div class="conquistas-grid">
          <div v-for="c in perfil.conquistas" :key="c.codigo" class="conquista-item">
            <span class="conquista-ico">{{ conquistaIco[c.codigo] || '🏆' }}</span>
            <span class="conquista-nome">{{ c.nome }}</span>
            <span class="muted" style="font-size:10px;">{{ new Date(c.conquistadaEm).toLocaleDateString('pt-BR') }}</span>
          </div>
        </div>
      </div>

      <!-- KPIs por tipo -->
      <div v-if="metricas?.porTipo?.length" class="grid" style="grid-template-columns: repeat(auto-fit, minmax(240px,1fr)); gap:16px;">
        <div v-for="t in metricas.porTipo" :key="t._id" class="card glow">
          <span class="badge" :class="'tipo-' + t._id" style="margin-bottom:12px;">{{ t._id }}</span>
          <div style="font-size:26px;font-weight:700;">
            {{ t.totalLidos ? ((t.totalConformes / t.totalLidos) * 100).toFixed(1) : 0 }}%
          </div>
          <div class="muted" style="font-size:12px;">conformidade</div>
          <div class="row mt-2" style="font-size:12px;">
            <span class="muted">Itens:</span><strong>{{ t.totalLidos?.toLocaleString('pt-BR') }}</strong>
            <span class="spacer" />
            <span class="muted">Pts:</span><strong>{{ Math.round(t.pontuacao) }}</strong>
          </div>
        </div>
      </div>
      ...
      <div class="card" v-if="metricas?.serie?.length">
        <div class="row mb-2">
          <h3 class="section-title" style="margin-bottom:0;">Itens auditados por dia</h3>
          <span class="spacer" /><fa :icon="serieComoColunas ? 'chart-bar' : 'chart-line'" class="muted" />
        </div>
        <AppChart :type="serieComoColunas ? 'bar' : 'line'" :data="serieChart" :height="260" :options="serieChartOptions" />
      </div>

      <div v-if="!metricas?.serie?.length" class="empty" style="padding:40px;">
        Nenhuma auditoria registrada ainda. Seus dados aparecerão aqui após o processamento das planilhas.
      </div>
    </div>
        ...
        <div class="crop-dialog">
          <div class="row justify-between items-center mb-2">
            <div>
              ...
              <p class="muted crop-copy">Arraste a imagem e defina o enquadramento do jeito que preferir.</p>
            </div>
          ...
          <div class="crop-stage">
            <img ref="cropperImageRef" :src="cropperImage" :alt="cropperNomeArquivo || 'Prévia do avatar'" class="crop-image" />
          </div>
...
.portal-shell {
  min-height: 100vh;
  background:
    radial-gradient(900px 600px at 5% -5%, rgba(124,92,255,.18), transparent 60%),
    radial-gradient(800px 600px at 100% 15%, rgba(34,211,238,.12), transparent 60%),
    var(--bg-0);
...
.brand-mark { width: 36px; height: 36px; border-radius: 10px; background: var(--grad-primary); display: grid; place-items: center; color: #fff; box-shadow: 0 6px 18px rgba(124,92,255,.4); font-size: 16px; }
.brand-mark.big { width: 64px; height: 64px; border-radius: 18px; font-size: 28px; margin: 0 auto; }
.brand-name { font-weight: 700; color: var(--text); }
.brand-name small { display: block; font-size: 11px; color: var(--text-dim); font-weight: 400; }
...
.theme-toggle.active {
  ...
  background: linear-gradient(180deg, rgba(124,92,255,.16), rgba(34,211,238,.08));
...
.avatar-big:hover { transform: scale(1.04); }
.avatar-overlay {
...
.conquista-ico { font-size: 28px; }
.conquista-nome { font-size: 12px; font-weight: 600; color: var(--text); }
...
:global([data-theme="light"]) .theme-toggle.active {
  background: linear-gradient(180deg, rgba(109,92,255,.14), rgba(17,197,255,.08));
}
